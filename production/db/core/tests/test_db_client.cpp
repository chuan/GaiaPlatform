...
TEST_F(db_client_test, gaia_ptr_no_transaction_fail)
{
    ...
    EXPECT_THROW(node1.insert_into_anchor_chain(1, 2), no_open_transaction);
    EXPECT_THROW(node1.remove_from_anchor_chain(1, 2), no_open_transaction);
    EXPECT_THROW(node1.remove_from_anchor_chain(2), no_open_transaction);
